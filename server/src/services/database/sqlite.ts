...
export class SQLiteDatabase implements IDatabase {
  ...
  createWebhookEvent(event: WebhookEventInput): number {
    ...
    // Use INSERT OR IGNORE to handle duplicate events idempotently
    // The UNIQUE constraint on (subscription_id, object_id, aspect_type, event_time)
    ...
    const stmt = db.prepare(`
      ...
      ON CONFLICT(subscription_id, object_id, aspect_type, event_time) DO NOTHING
    `);
    ...
    const result = stmt.run(
      ...
      event.objectId,
      event.aspectType,
      event.updatesJson ?? null,
      event.eventTime,
      now,
      now
    );

    // If no rows were inserted (duplicate), find the existing event ID
    if (result.changes === 0) {
  ...
  getUnprocessedWebhookEvents(limit = 100): StoredWebhookEvent[] {
    const db = this.getDb();
    const now = Math.floor(Date.now() / 1000);

    // Atomically claim pending events by marking them as 'processing'
    // This prevents multiple workers from grabbing the same events
    const claimTx = db.transaction((batchLimit: number) => {
      // Find pending events
      const findStmt = db.prepare(`
        SELECT id
        FROM webhook_events
        ...
        LIMIT ?
      `);
      const pendingIds = (findStmt.all(batchLimit) as Array<{ id: number }>).map(r => r.id);

      if (pendingIds.length === 0) {
        return [];
      }

      // Claim them by setting status to 'processing'
      ...
      const selectStmt = db.prepare(`
        ...
        ORDER BY created_at ASC
      `);
      return selectStmt.all(...pendingIds) as WebhookEventRow[];
    });

    const rows = claimTx(limit);
  ...
  markWebhookEventProcessed(eventId: number, errorMessage?: string): void {
    ...
    const status = errorMessage ? 'failed' : 'processed';
    ...
    stmt.run(status, now, errorMessage ?? null, now, eventId);
  }
  ...
  getWebhookEventsByActivity(
    ...
    const rows = stmt.all(ownerId, activityId) as WebhookEventRow[];
  ...
  cleanupOldWebhookEvents(olderThanSeconds = 30 * 24 * 60 * 60): number {
    const db = this.getDb();
  ...
  retryWebhookEvent(eventId: number, maxRetries = 3): boolean {
    const db = this.getDb();
    const now = Math.floor(Date.now() / 1000);

    // Only retry failed events that haven't exceeded max retries
    const stmt = db.prepare(`
      UPDATE webhook_events
      SET
        status = 'pending',
    ...
    const result = stmt.run(now, now, eventId, maxRetries);
    return result.changes > 0;
  }

  resetStuckWebhookEvents(timeoutSeconds = 300): number {
    const db = this.getDb();
    const now = Math.floor(Date.now() / 1000);
    const cutoffTime = now - timeoutSeconds;

    // Reset events that have been in 'processing' state too long
    ...
    const result = stmt.run(now, now, cutoffTime);
    return result.changes;
  }
}
